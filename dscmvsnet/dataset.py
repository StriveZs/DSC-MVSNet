...
# fixme: DTU Training set and validation set
...
class DTU_Test_Set(Dataset):
    ...
    def _load_dataset(self, dataset, lighting_set):
        path_list = []
        for ind in dataset:
            image_folder = osp.join(self.root_dir, "scan{}/images".format(ind))
            ...
            for lighting_ind in lighting_set:
                ...
                for p in range(0, int(self.cluster_list[0])):
                    ...
                    ref_index = int(self.cluster_list[22 * p + 1])
                    ref_image_path = osp.join(
                        image_folder, "{:0>8}.jpg".format(ref_index))
                    ref_cam_path = osp.join(cam_folder, "{:0>8}_cam.txt".format(ref_index))
                    ref_depth_path = osp.join(depth_folder, "depth_map_{:04d}.pfm".format(ref_index))
                    ...
                    for view in range(self.num_view - 1):
                        view_index = int(self.cluster_list[22 * p + 2 * view + 3])
                        view_image_path = osp.join(
                            image_folder, "{:0>8}.jpg".format(view_index))
                        view_cam_path = osp.join(cam_folder, "{:0>8}_cam.txt".format(view_index))
                        view_depth_path = osp.join(depth_folder, "depth_map_{:04d}.pfm".format(view_index))
                        view_image_paths.append(view_image_path)
                        view_cam_paths.append(view_cam_path)
                        view_depth_paths.append(view_depth_path)
...
# fixme: Tanks & Temples test set
class Tanks_Test_Set(Dataset):
    mean = torch.tensor([1.97145182, -1.52387525, 651.07223895])
    std = torch.tensor([84.45612252, 93.22252387, 80.08551226])
    ...
    def __init__(self, root_dir, dataset_name,
        ...
        self.width = width
        self.depth_folder = depth_folder
        self.cluster_file_name = "pair.txt"
        ...
        assert (dataset_name in ["tankandtemples"]), "Unknown dataset_name: {}".format(dataset_name)

        self.data_set = scans

        self.path_list = self._load_dataset(self.data_set)

    def _load_dataset(self, dataset):
        path_list = []
        for scan in dataset:
            image_folder = osp.join(self.root_dir, self.split + '/' +scan + "/images")
            cam_folder = osp.join(self.root_dir, self.split + '/' +scan +  "/cams_short")  # use short-range parameters
            depth_folder = osp.join(self.depth_folder, self.split + '/' + scan)
            cluster_file_path = osp.join(self.root_dir, self.split, scan, self.cluster_file_name)
...
def build_data_loader(cfg, mode="train"):
    if mode == "train":
        ...
        dataset = Tanks_Test_Set(
            root_dir=cfg.DATA.TEST.ROOT_DIR,
            dataset_name="tankandtemples",
            num_view=cfg.DATA.TEST.NUM_VIEW,
            height=cfg.DATA.TEST.IMG_HEIGHT,
            width=cfg.DATA.TEST.IMG_WIDTH,
            interval_scale=cfg.DATA.TEST.INTER_SCALE,
    ...
    # DataLoader set up, drop_last=True to avoid data load error
