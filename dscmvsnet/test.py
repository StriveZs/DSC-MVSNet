...
from model import build_pointmvsnet as build_model
...
os.environ['CUDA_VISIBLE_DEVICES']="0" # only use one GPU for test

def parse_args():
    parser = argparse.ArgumentParser(description="PyTorch DSC-MVSNet Evaluation")
    parser.add_argument(
        "--cfg",
        dest="config_file",
        default="./DSC-MVSNet/configs/dtu.yaml",
        metavar="FILE",
...
def test_model(model,
    ...
    logger = logging.getLogger("dscmvsnet.train")
...
def test(cfg, output_dir, isCPU=False):
    logger = logging.getLogger("dscmvsnet.tester")
    # build model
    ...
    # build data loader
    test_data_loader = build_data_loader(cfg, mode=cfg.DATA.TEST.MODE) # mode='test
    start_time = time.time()
    test_model(model,
...
def main():
    ...
    logger = setup_logger("dscmvsnet", output_dir, prefix="test")
    if isCPU:
